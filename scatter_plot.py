...
def find_max_correlation(corr_matrix):
    ...
    max_corr_value = (
        1  # Initialiser avec une valeur impossible pour une corrélation valide
    )
    max_indices = (0, 0)  # Initialiser les indices de la paire maximale
    # Parcourir la matrice de corrélation
    for i in range(corr_matrix.shape[0]):
        for j in range(
            i + 1, corr_matrix.shape[1]
        ):  # Eviter la diagonale et les duplicatas
...
def print_graph(data, a, b):
    ...
    for selection in selections:
        plt.scatter(
            data[data["Hogwarts House"] == selection][var1],
            data[data["Hogwarts House"] == selection][var2],
            alpha=0.5,
            label=selection,
        )
